...
function addTodo() {
  ...
  if (todoText.length > 0) {
    ...
    allTodos.push(todoObject); //pushes the value of the input field to the array
...
function createTodoItem(todo, todoIndex) {
  ...
  todoLI.innerHTML = `<input class="hidden" type="checkbox" id="${todoId}">
                ...
                ${todoText} //sets the innerHTML of the list item
                </label>
...
function deleteTodoItem(todoIndex) {
  allTodos = allTodos.filter((_, index) => index !== todoIndex); //deletes the todo item from the array
  saveTodos();
  updateTodoList(); //calls the function to update the unordered list
}
function saveTodos() {
